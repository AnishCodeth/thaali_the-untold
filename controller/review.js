...
const add_review = noTryCatch(async (req, res,next) => {
 ...
 console.log(req.user.username)
  ...
  let pgres=await client.query(`select * from review where r_username=$1 and username=$2 `,[req.body.r_username,req.body.username])
  let count=pgres.rows.length
  pgres=await client.query(`select * from payment where r_username=$1 and username=$2 `,[req.body.r_username,req.body.username])
  if(count>=pgres.rows.length)
  return next(new customError('You canonly review per payment but you can update',403))

 
  const { query, values } = await add_query(req.body, "review");
  await client.query(query, values);
  res.json("order added successfully");
});

const display_review = noTryCatch(async (req, res,next) => {
  const where_conditions = ["r_username","id","rating",'username'];
...
const update_review = noTryCatch(async (req, res,next) => {
  ...
  const where_conditions = ["r_username","id","rating",'username'];//to ensure what what can they update
...
const delete_review = async (req, res,next) => {
  const where_conditions = ["r_username",'username'];
  const client=await connectDB()
  const delete_payload=req.body
  delete_payload.username=req.user.username
