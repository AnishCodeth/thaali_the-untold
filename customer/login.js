...
const loginController = noTryCatch(async (req, res,next) => {
  ...
  let pgres = await client.query(
    `select * from customer_CREDENTIAL where (email=$1 or username=$2) and verified='verified'`,
...
const role='customer'
  ...
  return next(new customError('fill out field first',400))
  ...
  let create_table_query=`CREATE TABLE IF NOT EXISTS customer_CREDENTIAL (
    ...
    verified varchar(10) not null default 'verified' check(verified in ('pending','verified')),
    id serial)`;
...
const verified='verified';
  if (code == req.body.code) {
    ...
    let {query,values}=await add_query({email,username,password:e_password,verified},'customer_credential')
...
const updatePasswordController = noTryCatch(async (req, res) => {
  ...
  const pgres = await client.query(
    `select * from customer_credential where username=$1`,
    [username]
  );
  ...
  if (ispasswordsame) {
    ...
    await client.query(
      `update customer_credential set password=$1 where username=$2`,
...
const { email} = req.body;
  const client = await connectDB();
  let pgres = await client.query(
    `select * from customer_credential where email=$1`,
    [email]
  );
...
const resetpasswordController = noTryCatch(async (req, res) => {
  ...
  if (code == req.body.code) {
    ...
    const client = await connectDB();
    await client.query(
      `update customer_credential set password=$1 where email=$2`,
      [newPassword, email]
    );
