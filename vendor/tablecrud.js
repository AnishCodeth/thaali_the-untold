...
const add_table = noTryCatch(async (req, res) => {
  ...
  await client.query(
    ` create table if not exists FOOD_TABLE (
        ...
        r_username varchar(100) references vendor_CREDENTIAL(username) on delete cascade on update cascade,
...
const update_table = noTryCatch(async (req, res,next) => {
  const r_username=req.user.username
    if(!req.body.find)
    req.body.find={r_username}
  else
  req.body.find.r_username=r_username
